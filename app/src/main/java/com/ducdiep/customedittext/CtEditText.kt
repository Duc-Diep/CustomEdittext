    ...
    private fun initView() {
        ...
        edit_text.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(s: Editable?) {
                ...
                if (currentLength == maxLength + 1 && lastLength < currentLength && lastLength != 0) {//case lastLength < currentLength delete when length is max + 1
                    if (s.toString().indexOf(beforeText)!=0){
                        edit_text.filters = arrayOf(InputFilter.LengthFilter(maxLength))
                        edit_text.setText(beforeText)
                    ...
                    Log.d(
                        "length",
                        "afterTextChanged: currentfilter: $currentFilter, s: ${s.toString()}, edit text: ${edit_text.text}"
                    )
                } else {
